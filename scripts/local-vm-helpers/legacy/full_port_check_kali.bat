...
set "OUTPUT_FILE=%REPORTS_DIR%\kali_full_port_check_%TIMESTAMP%.txt"

echo Kali VM Full Port Check Report > "%OUTPUT_FILE%"
echo ================================================================ >> "%OUTPUT_FILE%"
...
echo Note: full TCP scans can take longer than the quick report. >> "%OUTPUT_FILE%"
echo. >> "%OUTPUT_FILE%"
...
call :RunSection "All TCP Ports on Localhost" "nmap -sT -sV -p- 127.0.0.1"
call :RunSection "All TCP Ports on VM IP" "nmap -sT -sV -p- %KALI_HOST%"

echo.
echo Full port check complete.
